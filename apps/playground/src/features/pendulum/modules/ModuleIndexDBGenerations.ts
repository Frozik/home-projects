...
import type { TDatabaseErrorCallback, TModuleApi } from '@frozik/utils';
import {
    createDatabase$,
    createDB,
    databaseReconnect,
...
import { DEDUPE_REMOVE_DELAY, SHARE_RESET_DELAY } from '../../../app/constants';
...
export const ModuleIndexDBGenerations = ModuleFactory(async () => {
        ...
        () =>
            createDatabase$<IDBCompetitions>(createGenerationDB).pipe(
                ...
                switchMap((database) =>
                    merge(
                        of(database),
                        receiveFromTabs(DATABASE_CHANNEL_SYNC_KEY).pipe(map(() => database)),
    ...
    return {
        getCompetitionsStarts$: dedobs(
            () => database$().pipe(switchMap((database) => from(getCompetitions(database)))),
            {
                resetDelay: SHARE_RESET_DELAY,
                removeDelay: DEDUPE_REMOVE_DELAY,
            },
        ),
        getGenerations$: dedobs(
            (competitionStart: ISO): Observable<IGeneration[]> =>
                database$().pipe(
                    switchMap((database) => from(getGenerations(database, competitionStart))),
                ),
            {
                normalize: ([competitionStart]) => competitionStart,
                resetDelay: SHARE_RESET_DELAY,
                removeDelay: DEDUPE_REMOVE_DELAY,
            },
        ),
        addGeneration$: (competitionStart: ISO, generation: IGeneration): Promise<void> =>
            firstValueFrom(
                database$().pipe(
                    switchMap((database) => addGeneration(database, competitionStart, generation)),
                    sendToTabs(DATABASE_CHANNEL_SYNC_KEY),
                    tap(() => databaseChanged$.next()),
                ),
            ),
        getRobot$: dedobs(
            (robotName: string) => {
                return database$().pipe(
                    switchMap((database) => from(getRobot(database, robotName))),
                    take(1),
                );
            },
            {
                normalize: ([robotName]) => robotName,
                resetDelay: SHARE_RESET_DELAY,
                removeDelay: DEDUPE_REMOVE_DELAY,
            },
        ),
...
export type TModuleIndexDBGenerations = TModuleApi<typeof ModuleIndexDBGenerations>;
...
export async function createGenerationDB(
    ...
    const requestedVersion = Math.max(
        (await getDatabaseVersion(DATABASE_NAME)) ?? 0,
