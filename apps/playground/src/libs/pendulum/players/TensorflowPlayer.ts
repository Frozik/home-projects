...
import type { IAction, IRobotPlayer, IWorld } from '../def';
import { EPlayerType } from '../def';
import { zNormalization } from '../utils';
import type { TLayerDescriptor } from './defs';
import { ELayerType, ENeuronLayerType } from './defs';
...
export class TensorflowPlayer implements IRobotPlayer {
    ...
    public getModelDescription(): TLayerDescriptor[] {
        ...
        tidy(() => {
            let leftNeuronIds: string[] | undefined = undefined;
                    ...
                    if (weight.shape.length === 2) {
                        ...
                        const currentLeftNeuronIds =
                            ...
                            Array(leftSize)
                                .fill(0)
                                .map(() => uuidv4());
                        const rightNeuronIds = Array(rightSize)
                            .fill(0)
                            .map(() => uuidv4());
                        ...
                        layers.push({
                            ...
                            axons: Array(leftSize * rightSize)
                                .fill(0)
                                .map((_, index) => {
    ...
    public async crossoverModels?(secondParent: IRobotPlayer): Promise<IRobotPlayer> {
        try {
            const newModel = await crossoverModels(
                this.model,
                (secondParent as TensorflowPlayer).model,
            );
    ...
    tidy(() =>
        fatherLayers.forEach((layer, index) => {
            ...
            const childWeights = fatherLayerWeights.map((fatherWeights, weightIndex) => {
                ...
                const fatherValues = (fatherWeights.arraySync() as number[]).slice(
                    0,
                    crossoverPoint,
                );
