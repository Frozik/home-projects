        ...
        const newSelection =
            fastInputSuffixes.length > 0
                ? {
                      start: Math.min(
                          newValue.length,
                          rawNewSelection.start + fastInputZeros.length,
                      ),
    ...
    return function rateTextToHtml(text: string, editing: boolean): string {
        ...
        const lettersMap = absText.split('').reduce(
            (acc, letter, index) => {
                ...
                if (letter === '.') {
                    acc.push({ position: NaN, letter });
                    processedDivider = true;
                } else {
                    acc.push({ position: letter === '.' ? NaN : position, letter });
        ...
        if (!processedDivider && (dividerIndex >= 0 || totalPosition < 0)) {
            lettersMap.push({ position: NaN, letter: '.' });
        }
        ...
        if (hasSign) {
            lettersMap.unshift({ position: NaN, letter: '-' });
        }
        ...
        return lettersMap
            .map(({ letter, position }) => {
                const classNames = [
                    ...
                    .join(' ');
                return isEmpty(classNames)
                    ? letter
                    : `<span class="${classNames}">${letter}</span>`;
            })
            .join('');
    };
}
...
export function setElementSelection(element: HTMLElement, selectionRange: ISelection): void {
    ...
    let startRange: { node: Node; offset: number } | undefined = undefined;
    let endRange: { node: Node; offset: number } | undefined = undefined;
    ...
    for (const textNode of traverseTextNodes(element)) {
        ...
        if (
            selectionRange.start >= currentOffset &&
            selectionRange.start <= currentOffset + length
        ) {
